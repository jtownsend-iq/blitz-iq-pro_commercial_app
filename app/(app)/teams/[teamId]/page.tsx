...
export default async function TeamPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  const targetTeamId = requestedTeamId ?? activeTeamId
  if (!targetTeamId) {
    redirect('/onboarding/team')
  }

  if (targetTeamId !== activeTeamId) {
    await supabase.from('users').update({ active_team_id: targetTeamId }).eq('id', user.id)
  }
  ...
  const {
    ...
    invites,
    positionGroups,
  } = await fetchTeamData(supabase, targetTeamId)
